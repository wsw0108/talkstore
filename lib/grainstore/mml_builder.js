...
//     `mapnik_version` is target version of mapnik, defaults to ``latest``
...
var MMLBuilder = function(redis_pool, opts, optional_args, init_callback){
        ...
        Step(
            ...
            function initCheck(err, data){
                ...
                do { 
                  ...
                  // save pre-recorded XML and interactivity
                  xml = _.isArray(record.xml) ? record.xml : [ record.xml ];
                  if ( record.interactivity ) interactivity = record.interactivity;
                  if ( record.sql ) opts.sql = record.sql;

                  if ( ! record.xml ) {
                    //console.log("Redis record doesn't have 'xml' field");
                    break; // no XML in record
                  }

                  if ( ! record.xml_version ) {
                    //console.log("No xml version in record");
                    break; // no xml_version in record
                  }

                  // XML target mapnik version mismatch
                  if ( record.xml_version != target_mapnik_version ) {
                    //console.log("xml version mismatch record");
                    break;
                  }

                  if ( ! _.isArray(record.xml) ) {
                    //console.log("'xml' field in redis record isn't an array");
                    break; // old format
                  }

                  // Need to refresh expiration (but not on simple access)
                  if ( ! opts.table && ! opts.token ) {
                    //console.log("need to refresh expiration date, interactivity_layer is " + interactivity_layer);
                    break;
                  }

                  // no interactivity xml for requested layer
                  if ( ! record.xml[interactivity_layer] ) {
                    //console.log("'xml' field doesn't have record " + interactivity_layer);
                    break;
                  }

                  //console.log("'xml' field has " + record.xml.length + " elements, interactivity_layer is " + interactivity_layer);
            ...
            function renderBaseStyleOrDefaultOrOverride(err, data){
                ...
                that.render(style, function(err, compiled_XML) {
                  ...
                  next(err, compiled_XML);
                }, style_version);
            },
            function setStore(err, compiled_XML){
                if (err) throw err;
                xml[interactivity_layer] = compiled_XML;
                var tostore = {xml: xml, xml_version: target_mapnik_version };
            ...
            function callbackExit(err, data){
                ...
                if (!_.isUndefined(redis_client))
                    redis_pool.release(grainstore_styles.db, redis_client);
                callback(err, JSON.stringify({style: style, version: style_version, xml: xml}));
            }
        );
    };
    ...
    me.touch = function(callback){
      if ( ! opts.token ) {
        callback(new Error("Only multilayer map configuration can be touched "));
        return;
      }
