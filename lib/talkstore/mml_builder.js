...
//
// params must have:
// `query`           - query condition to constrain the map by (can be an array), contains {dbname, layer, filter}
// `style`           - Carto style to override the built in style store (can be an array)
//
// params may have:
// `datasource_extend` - optional array of extra datasource options
// `extra_ds_opts`   - optional array of extra datasource default options
...
//     `datasource` specifies details from Mapnik maptalks plugin.
...
//     `mapnik_version` is target version of mapnik, defaults to ``3.0.7``
//     `mapnik_tile_format` for the tiles, see https://github.com/mapnik/mapnik/wiki/OutputFormats
//     `default_style_version` is the default version for CartoCSS styles. Defaults to '3.0.0'
//
//     eg.
//     {
//       map: {srid: 3857},
...
function MMLBuilder(params, options) {
    this.params = params || {};
    // core variables
    var requiredParams = ['query', 'style'];
    requiredParams.forEach(function(paramKey) {
        if (!params.hasOwnProperty(paramKey)) {
            throw new Error("Options must include '" + paramKey + "'");
        }
    });
    // TODO: check required [dbname, layer, filter]

    this.options = options || {};
    this.options.cachedir = this.options.cachedir || default_cache_dir;
    this.target_mapnik_version = options.mapnik_version || '3.0.7';
    this.default_style_version = options.default_style_version || '3.0.0';

    this.talkstore_datasource = _.defaults(options.datasource || {}, talkstore_defaults.datasource);

    this.talkstore_map = _.defaults(options.map || {}, talkstore_defaults.map);
...
MMLBuilder.prototype.baseMML = function() {
    ...
    for (var i=0; i<queries.length; ++i) {
        ...
        datasource.dbname = query.dbname;
